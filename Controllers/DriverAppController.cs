...
namespace SignalRHub
{
    ...
    public class DriverAppController : ApiController
    {
        ...
        private bool IsDriverBidding = false;
        ...
        private string MakePaymentCardTokenKonnectPay(Classes.KonnectPay.PaymentConfig obj, ClsPaymentInformation objCard, Booking objBooking)
        {
            string rtn = string.Empty;
        ...
        /// </summary>
        private string MakePaymentKonnectPay(Classes.KonnectPay.PaymentConfig obj, ClsPaymentInformation objCard, Booking objBooking, bool IsAuthorize)
        {
            string rtn = string.Empty;
            string DefaultCurrency = System.Configuration.ConfigurationManager.AppSettings["DefaultCurrency"];
            ...
            try
            {
                if (objCard.TokenDetails.ToStr().StartsWith("pi_") && IsAuthorize)
                {
                    ...
                    st.key = "";
                    st.secret = "";
                    st.amount = Convert.ToInt64(objCard.Price * 100);
                    st.displayAmount = objCard.Price.ToDecimal();
                    st.currency = DefaultCurrency;
                    st.defaultClientId = HubProcessor.Instance.objPolicy.DefaultClientId.ToStr();
                    st.location = DefaultClientLocation;
        ...
        /// </summary>
        private dynamic GetKonnectPayGatewayDetails(Classes.KonnectPay.PaymentConfig obj, long DriverID)
        {
            dynamic resp = null;
            try
            {
        ...
        /// </summary>
        private ResponseData CreatePaymentLinkKonnectPay(Classes.KonnectPay.PaymentConfig obj, Booking objBooking, string sendtype, string mobileno, string email, decimal Price)
        {
            ResponseData resp = new ResponseData();
            string DefaultCurrency = System.Configuration.ConfigurationManager.AppSettings["DefaultCurrency"];
            string DefaultClientLocation = System.Configuration.ConfigurationManager.AppSettings["DefaultClientLocation"];
        ...
        /// </summary>
        private ResponseData GetPaymentLinkKonnectPay(Classes.KonnectPay.PaymentConfig obj, Booking objBooking, decimal Price, bool IsPayFromDriverApp)
        {
            ResponseData resp = new ResponseData();
            string DefaultCurrency = System.Configuration.ConfigurationManager.AppSettings["DefaultCurrency"];
            string DefaultClientLocation = System.Configuration.ConfigurationManager.AppSettings["DefaultClientLocation"];
        ...
        public ResponseData requestupcomingjoblist(string mesg)
        {
            try
            {
                ...
                File.AppendAllText(physicalPath + "\\" + "requestupcomingjoblist.txt", DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + ", json: " + mesg.ToString() + Environment.NewLine);
            }
            catch
            {
            ...
            try
            {
                ...
                int driverId = int.Parse(objAction.DrvId);
                string driverNo = objAction.DrvNo;
        ...
        public ResponseData requestdriverJobBid(string mesg)
        {
            ...
            try
            {
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    ...
                    if (objBooking != null && objBooking.BookingStatusId == Enums.BOOKINGSTATUS.WAITING)
                    {
                        ...
                        else
                        {
                            ...
                            //    obj.ElapsedTime = elapedTime;
                            ////objBooking.BookingStatusId = Enums.BOOKINGSTATUS.PENDING_START;
                            ////objBooking.DriverId = driverId;
                            ////db.SubmitChanges();
                            //Global.listofDrvBidding.Add(obj);
                            IsDriverBidding = true;
                            General.OnDespatching(HubProcessor.Instance.objPolicy, objBooking, db.Fleet_Drivers.FirstOrDefault(c => c.Id == driverId), 3, false, "system");
            ...
            try
            {
                ...
                using (TaxiDataContext db = new TaxiDataContext())
                {
                    ...
                    else
                    {
                        ...
                        var DriverNo=db.Fleet_Drivers.FirstOrDefault(x=>x.Id == values[1].ToInt());
